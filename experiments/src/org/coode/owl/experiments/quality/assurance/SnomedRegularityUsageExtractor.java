...
public class SnomedRegularityUsageExtractor {
    ...
    /** @param args
     *            args[0]:inferred snomed ontology, args[1]: keyword */
    public static void main(String[] args) {
        if (args.length >= 1) {
            String ontologyName = args[0];
            System.out.println("Loading the ontology...");
            OWLOntology onto = ExperimentUtils.loadOntology(new File(ontologyName));
            List<String> keywords = ExperimentUtils
                    .extractListFromFile("processedKeywords.txt");
            for (String keyword : keywords) {
                System.out.println(String.format("Extracting usage for %s keyword",
                        keyword));
                ...
                if (!ontoFile.exists()) {
                    ...
                    System.out.println("Ontology was saved in " + ontoFile.getPath());
                }
            }
        } else {
            System.out.println(String.format("Usage java -cp ... %s <ontology.owl>",
                    SnomedRegularityUsageExtractor.class.getCanonicalName()));
        }
    }

    private static Set<OWLAxiom> extractKeywordUsage(String keyword, OWLOntology onto) {
        ...
        System.out.println("Found " + target.size() + " " + keyword + " entities "
                + "and " + " " + axioms.size() + " axioms.");
        return axioms;
    }
}
