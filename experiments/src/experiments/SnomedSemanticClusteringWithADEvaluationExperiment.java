...
public class SnomedSemanticClusteringWithADEvaluationExperiment extends
	...
	public static void main(String[] args) throws Exception {
		...
		setupClusteringExperimentPopularity(pop, i2, new File(root + i2 + pop
				+ suffix));
		setupClusteringExperimentStructural(struc, i2, new File(root + i2
				+ struc + suffix));
		...
		setupClusteringExperimentPopularity(pop, i1, new File(root + i1 + pop
				+ suffix));
		setupClusteringExperimentStructural(struc, i1, new File(root + i1
				+ struc + suffix));
		setupClusteringExperimentProperty(prop, i1, new File(root + i1 + prop
				+ suffix));
		setupClusteringExperimentStructural(struc, i0, new File(root + i0
				+ struc + suffix));
	...
	public static void setupClusteringExperimentPopularity(String type,
		...
		if (!f.exists()) {
			...
			OWLOntology o = m.loadOntologyFromOntologyDocument(new File(base
					+ current));
