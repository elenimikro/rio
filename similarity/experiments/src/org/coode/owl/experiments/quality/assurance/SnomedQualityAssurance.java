...
public class SnomedQualityAssurance {
	...
	private final static String chronic_snomed_2013 = "snomed/chronic_inferred_usage_2013.owl";
	private final static String acute_snomed_2013 = "snomed/acute_inferred_usage_2013.owl";
		...
		try {
			if (args.length >= 2) {
				...
				if (!xml.exists()) {
					...
					System.out.println("Regularity results were saved in "
							+ xml);
					System.out.println("Computing quality assurance stats...");
					o.getOWLOntologyManager().addAxioms(o, annotations);
					ArrayList<SimpleMetric<?>> stats = qa
							.getQualityAssuranceStats(model);
