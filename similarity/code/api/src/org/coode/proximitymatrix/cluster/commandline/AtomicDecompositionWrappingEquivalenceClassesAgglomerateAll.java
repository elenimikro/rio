...
public class AtomicDecompositionWrappingEquivalenceClassesAgglomerateAll {
	...
	public static void main(String[] args) {
		if (args.length >= 2) {
			...
			//RelevancePolicy<OWLEntity> relevancePolicy = DefaultOWLEntityRelevancePolicy.getAlwaysRelevantPolicy(); 
			//AtomicDecompositionRelevancePolicy relevancePolicy = new AtomicDecompositionRelevancePolicy(axiom, dataFactory, ontologies, axiomMap, entityAtomDependencies)
			final Distance<OWLEntity> distance = new AxiomRelevanceAtomicDecompositionBasedDistance(manager.getOntologies(),
			...
			final SimpleProximityMatrix<DistanceTableObject<OWLEntity>> wrappedMatrix = new SimpleProximityMatrix<DistanceTableObject<OWLEntity>>(
					...
					new Distance<DistanceTableObject<OWLEntity>>() {
						public double getDistance(DistanceTableObject<OWLEntity> a,
							...
							return distanceMatrix.getEntry(a.getIndex(), b.getIndex());
						}
					});
...
//					.build(leftOvers));
			...
			System.out
					.println(String
							.format("Usage java -cp ... %s <saveResultFilePath> <ontology> ... <ontology>",
									AtomicDecompositionWrappingEquivalenceClassesAgglomerateAll.class
											.getCanonicalName()));
		}
	}
	...
	private static <P> Set<Cluster<P>> buildClusters(
		...
		Set<Collection<? extends DistanceTableObject<P>>> objects = clusteringMatrix
				.getObjects();
