	...
	public static void main(String[] args) {
		if (args.length >= 2) {
			...
			System.out.println(String.format(
					...
					clusters.size()));
			//History<Collection<? extends DistanceTableObject<OWLEntity>>> history = clusteringMatrix.getHistory();
			save(clusters, manager, outfile);
		} else {
			System.out
					.println(String
							.format("Usage java -cp ... %s <saveResultFilePath> <ontology> ... <ontology>",
									AtomicDecompositionDifferenceWrappingEquivalenceClassesAgglomerateAll.class
											.getCanonicalName()));
		}
	}
