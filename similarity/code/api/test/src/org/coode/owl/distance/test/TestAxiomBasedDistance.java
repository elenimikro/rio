...
public class TestAxiomBasedDistance extends DistanceTestCase {
	...
	public void testPizza() {
		...
		AbstractAxiomBasedDistance distance = getDistanceBuilder().getDistance(
				o, DefaultOWLEntityRelevancePolicy.getAlwaysIrrelevantPolicy());
	...
	public void testMargheritaSundriedTomatoTopping() {
		OWLOntology o = TestHelper.getPizza();
		AbstractAxiomBasedDistance distance = getDistanceBuilder().getDistance(
				o, DefaultOWLEntityRelevancePolicy.getAlwaysRelevantPolicy());
	...
	public void testMargheritaSiciliana() {
		...
		OWLClass[] classes = getClasses(pizza_ns + "Margherita", pizza_ns
				+ "Siciliana");
		properTest(distance, classes);
		System.out.println(distance.getDistance(classes[0], classes[1]));
	}
	...
	public void testUnclosedPizzaIceCream() {
		...
		OWLClass[] classes = getClasses(pizza_ns + "UnclosedPizza", pizza_ns
				+ "IceCream");
		properTest(distance, classes);
	}
	...
	public void testMargheritaSicilianaPopularityRelevance() {
		OWLOntology o = TestHelper.getPizza();
		OWLEntityPopularityRanking ranking = OWLEntityPopularityRanking
				.buildRanking(o.getImportsClosure());
		...
		for (RankingSlot<OWLEntity> s : ranking.getSortedRanking()) {
			System.out.println(String.format("%d. %s value %s is relevant: %b",
					i, Arrays.toString(s.getMembers()), s.getValue(),
	...
	public void testSpicinessSauceToppingPopularityRelevance() {
		...
		AbstractAxiomBasedDistance distance = getDistanceBuilder().getDistance(
				o, policy);
		OWLClass[] classes = getClasses(pizza_ns + "Spiciness", pizza_ns
				+ "SauceTopping");
		properTest(distance, classes);
		int i = 1;
		System.out.println(String.format("Average popularity %s",
	...
	public void testToyOntology() {
		...
		System.out.println(String.format(
				"Average popularity %s standard deviation %f",
