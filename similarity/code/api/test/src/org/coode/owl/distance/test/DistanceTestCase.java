		...
		for (int i = 0; i < entities.length; i++) {
			for (int j = 0; j < entities.length; j++) {
				...
				if (i == j) {
					...
					Collection<OWLAxiom> axioms_i = distance
							.getAxioms(entities[i]);
					Collection<OWLAxiom> axioms_j = distance
							.getAxioms(entities[j]);
					Collection<OWLAxiom> union = distance
							.getAxioms(entities[i]);
					...
					if (axioms_i.isEmpty()) {
						...
						assertTrue(Double.compare(
								distance.getDistance(entities[j], entities[i]),
								1D) == 0);
					} else {
						double inverse_d = distance.getDistance(entities[j],
								entities[i]);
						System.out.println("Intersection between "
								+ entities[i] + " and " + entities[j]
								+ " of size " + axioms_i.size());
						for (OWLAxiom ax : axioms_i) {
							System.out.println(ax);
						}
						System.out.println();
						System.out.println("Union between " + entities[i]
								+ " and " + entities[j] + " of size "
								+ union.size());
