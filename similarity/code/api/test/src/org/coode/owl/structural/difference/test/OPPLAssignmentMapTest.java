...
public class OPPLAssignmentMapTest extends TestCase{
	...
	public void testAsignmentMapDisjointness() throws OWLOntologyCreationException, OPPLException{
		...
		VariableType<?> variableType = VariableTypeFactory
                .getVariableType(a);
		InputVariable<?> var = constraintSystem.createVariable(
                "?x", variableType, null);
		InputVariable<?> anothervar = constraintSystem.createVariable(
                "?y", variableType, null);
		...
		anotherBindingNode.addAssignment(new Assignment(anothervar, a));
		...
		maps.add(map);
		System.out
		.println("OPPLAssignmentMapTest.testAsignmentMapDisjointness() maps " + maps);
