...
public class ClusterCreatorTest {
	...
	@Before
	public void setUp() throws OPPLException, ParserConfigurationException {
		...
		clusters = clusterer.agglomerateAll(ontology, distance, entities);
	}
	...
	public void testClusteringMatrix() {
		...
		for (Collection<? extends DistanceTableObject<OWLEntity>> anObject : objects) {
			for (Collection<? extends DistanceTableObject<OWLEntity>> anotherObject : objects) {
				double d = clusteringMatrix
						.getDistance(anObject, anotherObject);
				assertFalse(d < 0);
			}
		}
	}
	...
	public void testClusteringStats() {
		...
		System.out
				.println("ClusterCreatorTest.testClusteringStats() Homogeneity "
