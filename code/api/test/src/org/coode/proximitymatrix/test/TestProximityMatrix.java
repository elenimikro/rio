...
public class TestProximityMatrix {
	...
	public void testAgglomerate() {
		...
		PairFilter<Collection<? extends OWLEntity>> filter = new PairFilter<Collection<? extends OWLEntity>>() {
			...
			public boolean accept(final Collection<? extends OWLEntity> first,
				...
				while (!found && iterator.hasNext()) {
					...
					Iterator<? extends OWLEntity> anotherIterator = second
							.iterator();
		...
		assertTrue(clusteringMatrixObjects.size() == distanceMatrixObjects
				.size());
		...
		for (OWLEntity owlEntity : signature) {
			for (OWLEntity anotherOWLEntity : signature) {
				...
				if (minimumDistancePair.contains(Collections
						.singleton(owlEntity))
				...
				assertTrue(
						String.format(
								...
								owlEntity, distanceMatrix
										.getRowIndex(owlEntity), agglomerated
										.getRowIndex(Collections
												.singleton(owlEntity)),
								anotherOWLEntity, distanceMatrix
										.getColumnIndex(anotherOWLEntity),
